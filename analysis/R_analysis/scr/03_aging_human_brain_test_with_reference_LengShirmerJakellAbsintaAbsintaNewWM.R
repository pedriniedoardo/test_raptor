...
ref03 <- readRDS("../../out/object/pseudobulk_leng_wholeSample_TPM_filter_ensembl.rds")
colnames(ref03) <- paste0("leng.",colnames(ref03))
ref04 <- readRDS("../../out/object/pseudobulk_abisntaNewWM_wholeSample_TPM_filter_ensembl.rds")
colnames(ref04) <- paste0("absintaNewWM.",colnames(ref04))
...
LUT_ref03 <- read_tsv("../../out/table/meta_full_leng.tsv") %>% 
  group_by(pseudobulk2,age) %>% 
  summarise() %>% 
  ungroup() %>%
  mutate(pseudobulk2_fix = paste0("leng.",pseudobulk2)) %>%
  # slice_sample(n = 20) %>% 
  # order the meta as in the column of the df_test
  dplyr::slice(match(colnames(ref03),.$pseudobulk2_fix))

LUT_ref04 <- read_tsv("../../out/table/meta_full_absintaNewWM.tsv") %>% 
  group_by(pseudobulk2,age) %>% 
  summarise() %>% 
  ungroup() %>%
  mutate(pseudobulk2_fix = paste0("absintaNewWM.",pseudobulk2)) %>%
  # slice_sample(n = 20) %>% 
  # order the meta as in the column of the df_test
  dplyr::slice(match(colnames(ref04),.$pseudobulk2_fix))
...
# try to stage also all the samples from leng -----------------------------
# quantile normalization of the matrices
df_ref03_norm <- log1p(limma::normalizeBetweenArrays(ref03, method = "quantile"))

# stage samples on BA47 & BA11 references
# use r47 as reference
ae_df_ref03_norm_r_df_ref <- ae(df_ref03_norm, r_df_ref$interpGE, r_df_ref$time.series)

# add the estimated ages to the original dataset as metadata
LUT_ref03$ae_r_df_ref <- ae_df_ref03_norm_r_df_ref$age.estimates[, 1]

# plotting ----------------------------------------------------------------
LUT_ref03 %>% 
  mutate(delta_age = ae_r_df_ref - age) %>% 
  separate(pseudobulk2,into = c("pathology","disease","sample"),sep = "\\.",remove = F) %>% 
  mutate(pathology = factor(pathology,levels = c("CTRL","ADB2","ADB6"))) %>% 
  ggplot(aes(x=pathology,y=delta_age)) + 
  geom_boxplot(outlier.shape = NA) +
  geom_point(position = position_jitter(width = 0.2),alpha = 0.5)+theme_bw()+geom_hline(yintercept = 0,linetype = "dashed",col="gray")


# try to stage also all the samples from absinta new wm -------------------
# quantile normalization of the matrices
df_ref04_norm <- log1p(limma::normalizeBetweenArrays(ref04, method = "quantile"))

# stage samples on BA47 & BA11 references
# use r47 as reference
ae_df_ref04_norm_r_df_ref <- ae(df_ref04_norm, r_df_ref$interpGE, r_df_ref$time.series)

# add the estimated ages to the original dataset as metadata
LUT_ref04$ae_r_df_ref <- ae_df_ref04_norm_r_df_ref$age.estimates[, 1]
...
list(LUT_Absinta = LUT_Absinta %>%
       dplyr::select(pseudobulk2,age=age.x,pseudobulk2_fix,ae_r_df_ref),
     LUT_jakell = LUT_ref01,
     LUT_shirmer = LUT_ref02,
     LUT_AbsintaNewWM = LUT_ref04) %>% 
  ...
  ggplot(aes(x=pathology,y=delta_age)) + 
  geom_boxplot(outlier.shape = NA) +
  geom_point(position = position_jitter(width = 0.2),alpha = 0.5)+theme_bw()+geom_hline(yintercept = 0,linetype = "dashed",col="gray")+facet_wrap(~dataset,scales = "free_x")+theme(strip.background = element_blank(),axis.text.x = element_text(hjust = 1,angle = 45))+ggtitle("reference Leng Absinta AbsintaNewWM Jakell & Shirmer")
ggsave("../../out/image/RAPTor_reference_LengAbsintaJakellShirmerAbsintaNewWM.pdf",width = 6,height = 6)

# save the full meta of the age and of the reference
meta_ref %>% 
  write_tsv("../../out/table/meta_ref_LengAbsintaJakellShirmerAbsintaNewWM.tsv")
...
list(LUT_Absinta = LUT_Absinta %>%
  ...
  bind_rows(.id = "dataset") %>% 
  mutate(delta_age = ae_r_df_ref - age) %>% 
  write_tsv("../../out/table/table_EstimateAge_LengAbsintaJakellShirmerAbsintaNewWM.tsv")
