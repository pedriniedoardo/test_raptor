...
data.combined.all <- readRDS("../../data/schirmer.rds")
...
# # fix the pathology column
  mutate(pathology_fix = case_when(pathology == "control"~"CTRL",
                                   pathology == "chronic_active"~"CA",
                                   pathology == "chronic_inactive"~"CI"
                                   ))
...
data.combined.all$disease <- case_when(data.combined.all$pathology=="control"~"ctrl",
                                       T~"ms")
...
# save the matrix as object
matrix_counts_alt %>%
  saveRDS("../../out/object/pseudobulk_shirmer_counts_wholeSample_raw.rds")
matrix_counts2_alt %>%
  saveRDS("../../out/object/pseudobulk_shirmer_counts_wholeSample_norm.rds")

# save the final meta
write_tsv(meta_full,file = "../../out/table/meta_full_shirmer.tsv")
