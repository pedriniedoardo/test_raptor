...
data.combined.all <- readRDS("../../data/all20_integrated_clean_metadata.rds")
...
# read in the LUT
LUT_sample <- read_tsv("../../data/LUT_sample_metabolite_RNA.tsv")
...
meta_full <- meta %>%
  ...
  left_join(LUT_sample,by = c("sample_number_fix"))

# add the new metadata
data.combined.all$clusterCellType <- meta_full$clusterCellType
data.combined.all$pathology_fix <- meta_full$pathology_fix
data.combined.all$sample_number_fix <- meta_full$sample_number_fix
...
# confirm the groupign for the metadata
table(data.combined.all$patient,data.combined.all$sample_number_fix)
...
# save the matrix as object
matrix_counts_alt %>%
  saveRDS("../../out/object/pseudobulk_all20_integrated_clean_metadata_counts_wholeSample_raw.rds")
matrix_counts2_alt %>%
  saveRDS("../../out/object/pseudobulk_all20_integrated_clean_metadata_counts_wholeSample_norm.rds")

# save the final meta
write_tsv(meta_full,file = "../../out/table/meta_full.tsv")
