...
ref04 <- readRDS("../../out/object/pseudobulk_absintaCX_wholeSample_TPM_filter_ensembl.rds")
colnames(ref04) <- paste0("absintaCX.",colnames(ref04))
...
LUT_ref04 <- read_tsv("../../out/table/meta_full_absintaCX.tsv") %>% 
  group_by(pseudobulk2,age) %>% 
  summarise() %>% 
  ungroup() %>%
  mutate(pseudobulk2_fix = paste0("absintaCX.",pseudobulk2)) %>%
  # slice_sample(n = 20) %>% 
  # order the meta as in the column of the df_test
...
LUT_ref04 %>% 
  mutate(delta_age = ae_r_df_ref - age) %>% 
  separate(pseudobulk2,into = c("pathology","disease","sample"),sep = "\\.",remove = F) %>% 
  mutate(pathology = factor(pathology,levels = c("CTRL","MS"))) %>% 
  ggplot(aes(x=pathology,y=delta_age)) + 
  geom_boxplot(outlier.shape = NA) +
  geom_point(position = position_jitter(width = 0.2),alpha = 0.5)+theme_bw()+geom_hline(yintercept = 0,linetype = "dashed",col="gray")
...
list(LUT_Absinta = LUT_Absinta %>%
       dplyr::select(pseudobulk2,age=age.x,ae_r_df_ref),
  ...
  ggplot(aes(x=pathology,y=delta_age)) + 
  geom_boxplot(outlier.shape = NA) +
  geom_point(position = position_jitter(width = 0.2),alpha = 0.5)+theme_bw()+geom_hline(yintercept = 0,linetype = "dashed",col="gray")+facet_wrap(~dataset,scales = "free_x")+theme(strip.background = element_blank(),axis.text.x = element_text(hjust = 1,angle = 45))+ggtitle("reference Leng AbsintaWM AbisntaCX Jakell & Shirmer")
ggsave("../../out/image/RAPTor_reference_LengAbsintaJakellShirmerAbsintaCX.pdf",width = 9,height = 5)
